...
foot = [0 0 0; 0.6 0.15 0.06; 0.9 0.3 -0.03; 1.2 0.45 0.12; 1.5 0.3 0.03; 1.8 0.2 0.18; 2.1 0.1 0.09; 2.4 0 0.15; 100 0 0];
% 何歩目に何秒かける？ 歩幅は一緒 最後は終了を示すフラグ
...
Q = zeros(4);
Q(1) = 10^300;                            %default: 10^3 ★★★最適制御の重み係数　大：ZMPが理想に近づく
...
for tt = t
  i = i + 1;
% fd = ((H+G'*P*G)^(-1))*G'*(xi')^1*P*Gd;	% 外乱除去用未使用
  px = C_d*x;                           % 実際のZMP位置
  py = C_d*y;
